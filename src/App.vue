...
export default {
  ...
  methods: {
    ...
    weixinAutoLogin() {
      let canAuto = this.$userInfo.isWeiXin && !this.$userInfo.isLogin && !getCookie('user_uid');
      if (canAuto) {
        window.location.href = `/apis/0.1/User/wxLogin.php?backurl=${encodeURIComponent(window.location.href)}`;
      }
    },
    isWeiXinFun() {
  ...
  created() {
    ...
    this.$nextTick(() => {
      ...
      if (!this.$userInfo.isLogin && this.$userInfo.isWeiXin) {
        axios
          .get(`/apis/0.1/User/Msg.php?a=user&backurl=${encodeURIComponent(window.location.href)}`)
          .then(res => {});
      }
    });
  }
