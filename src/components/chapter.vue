			...
			<div class="read-content-content" @click="menuSetFlag=!menuSetFlag" :style="{fontSize:fontSize+'px'}">
				...
				<div class="purchase-wraper">
					...
					<div class="wraper-content">
						<p class="pay-tit"><span>购买：{{chapter.chapter_name}}</span></p>
						<p class="pay-price"><span>价格：</span> <span>{{chapter.price}}书币</span></p>
						<p class="pay-tit"><span>剩余书币：{{chapter.chapter_name}}</span></p>
						<p class="pay-tit"><span>剩余书券：{{chapter.chapter_name}}</span></p>
						<div class="auto-pay">
					...
					<el-button type="primary" style="width:80%;margin:0 auto;display:block" @click.stop.native="buyChapter">立刻购买</el-button>
				</div>
...
export default {
	...
	methods: {
		...
		buyChapter(){
			...
			axios.post('/apis/0.1/Chapter/BuyRead.php',qs.stringify(data)).then(res=>{
