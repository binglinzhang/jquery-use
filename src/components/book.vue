        ...
        <div class="content">
          ...
          <div class="info">
            ...
            <div class="read-btn">
              ...
              <div class="btn">
                <a class="shelf" @click="toggleBookSelf">{{book.bookself==1?'加入书架':'移出书架'}}</a>
              </div>
            </div>
            <!---->
          </div>
...
export default {
	...
	created() {
		...
		axios.get(`/apis/0.1/BookInfo.php?bookId=${bookId}`).then(res => {
	...
	methods: {
		toggleBookSelf() {
			let auto = this.oneTwoToggle(this.book.bookself);
			axios.get(`/apis/0.1/Bookself.php?book_id=${this.$route.query.bookId}&auto=${auto}`).then(res=>{
				if(res.data.status==0){
					this.book.bookself = this.oneTwoToggle(this.book.bookself);
				}else if(res.data.status==201){
					this.$turnToLogin(res.data.msg,'book')
				}
			})
		},
		clearScrollTop() {
